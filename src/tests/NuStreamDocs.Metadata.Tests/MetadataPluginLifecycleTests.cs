...
public class MetadataPluginLifecycleTests
{
    ...
    {
        await new MetadataPlugin().OnConfigureAsync(new(default, "/in", "/out", []), CancellationToken.None);
    }
    ...
    {
        await new MetadataPlugin().OnFinaliseAsync(new("/out"), CancellationToken.None);
    }
