...
public class Utf8RelativePathTests
{
    ...
    /// <returns>Async test.</returns>
    [Test]
    public async Task TargetInsideFromIsSibling() => await Assert.That(Compute("articles", "articles/post.md")).IsEqualTo("post.md");

    /// <summary>A target outside the source directory walks up via <c>../</c> for each unmatched segment.</summary>
    /// <returns>Async test.</returns>
    [Test]
    public async Task TargetOutsideFromUsesParentSegments() => await Assert.That(Compute("articles/tags", "articles/post.md")).IsEqualTo("../post.md");

    /// <summary>Two unmatched segments produce two <c>../</c> hops.</summary>
    /// <returns>Async test.</returns>
    [Test]
    public async Task TwoUnmatchedSegmentsTwoParents() => await Assert.That(Compute("articles/tags/2024", "other/post.md")).IsEqualTo("../../../other/post.md");

    /// <summary>A sibling-directory target needs one parent hop and the target's directory.</summary>
    /// <returns>Async test.</returns>
    [Test]
    public async Task SiblingDirectoryTarget() => await Assert.That(Compute("blog/posts", "blog/category/dotnet.md")).IsEqualTo("../category/dotnet.md");

    /// <summary>An empty source directory passes the target through unchanged.</summary>
    /// <returns>Async test.</returns>
    [Test]
    public async Task EmptyFromPassesTargetThrough() => await Assert.That(Compute(string.Empty, "post.md")).IsEqualTo("post.md");

    /// <summary>An empty source directory and empty target both collapse to a self-reference.</summary>
    /// <returns>Async test.</returns>
    [Test]
    public async Task EmptyFromAndTargetSelfReference() => await Assert.That(Compute(string.Empty, string.Empty)).IsEqualTo(".");

    /// <summary>A subdirectory target one level deeper resolves without parent hops.</summary>
    /// <returns>Async test.</returns>
    [Test]
    public async Task SubdirectoryTargetIsBareRelativePath() => await Assert.That(Compute("blog", "blog/posts/intro.md")).IsEqualTo("posts/intro.md");
