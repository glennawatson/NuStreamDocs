...
public class LocalFontProviderTests
{
    ...
    public async Task ResolvesGlobbedFiles()
    {
        ...
        var resources = await LocalFontProvider.Instance.ResolveAsync(face, [], new FontDownloadCache(dir.Root, offline: true), dir.Root, subsetUsage: null, CancellationToken.None);
        ...
        await Assert.That(System.Text.Encoding.UTF8.GetString(regular.FamilyBytes)).IsEqualTo("MyFont");
    }
    ...
    public async Task NoMatchThrows()
    {
        ...
        try
        {
            await LocalFontProvider.Instance.ResolveAsync(face, [], new FontDownloadCache(dir.Root, offline: true), dir.Root, subsetUsage: null, CancellationToken.None);
        }
