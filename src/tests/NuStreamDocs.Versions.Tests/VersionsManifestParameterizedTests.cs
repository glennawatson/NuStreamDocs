...
/// <summary>Parameterized round-trip / read-tolerance tests for VersionsManifest.</summary>
public class VersionsManifestParameterizedTests
{
    ...
    private sealed class ScratchDir : IDisposable
    {
        ...
        public ScratchDir()
        {
            Root = Path.Combine(Path.GetTempPath(), $"smkd-vmp-{Guid.NewGuid():N}");
