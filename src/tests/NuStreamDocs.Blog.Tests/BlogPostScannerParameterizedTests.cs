...
/// <summary>Parameterized filename + frontmatter shape tests for BlogPostScanner.</summary>
public class BlogPostScannerParameterizedTests
{
    ...
    /// <summary>Slug variants humanize into title-cased space-separated names.</summary>
    /// <param name="slug">Slug part of the filename.</param>
    /// <param name="expected">Expected humanized title.</param>
    /// <returns>Async test.</returns>
    [Test]
    ...
    private sealed class ScratchDir : IDisposable
    {
        ...
        public ScratchDir()
        {
            Root = Path.Combine(Path.GetTempPath(), $"smkd-bpsp-{Guid.NewGuid():N}");
