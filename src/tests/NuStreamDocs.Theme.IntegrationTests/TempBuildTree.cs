...
/// <summary>Disposable docs/site directory pair used by the theme integration tests.</summary>
internal sealed class TempBuildTree : IDisposable
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "smkd-itheme-" + Guid.NewGuid().ToString("N", CultureInfo.InvariantCulture));
