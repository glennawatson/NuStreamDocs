...
/// <summary>Parameterized inputs for HeadingScanner.Scan covering heading level, id quoting, and stop-byte branches.</summary>
public class HeadingScannerParameterizedTests
{
    /// <summary>Heading levels h1..h6 are all recognized.</summary>
    ...
    [Arguments(5)]
    [Arguments(6)]
    public async Task HeadingLevelRecognized(int level)
    {
    ...
    public async Task QuotedIdRoundTrips(char quote)
    {
        ...
        var headings = HeadingScanner.Scan(html);
        await Assert.That(headings[0].ExistingIdBytes(html).SequenceEqual("target"u8)).IsTrue();
    }

    /// <summary>Unquoted id stops at every recognized stop byte.</summary>
