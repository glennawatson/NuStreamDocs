...
internal static partial class FontsLogging
{
    ...
    [LoggerMessage(EventId = 1, Level = LogLevel.Information,
        Message = "Resolved font family '{family}' ({fileCount} files)")]
    internal static partial void LogFontResolved(ILogger logger, string family, int fileCount);

    /// <summary>Logs that no metrics could be read for a family, so its CLS fallback override was skipped.</summary>
    /// <param name="logger">Diagnostic logger.</param>
    /// <param name="family">CSS family name.</param>
    [LoggerMessage(EventId = 2, Level = LogLevel.Warning,
        Message = "Could not read metrics for font family '{family}'; emitting it without a CLS fallback override")]
    internal static partial void LogMetricsUnavailable(ILogger logger, string family);
    ...
    internal static partial void LogStylesheetWritten(ILogger logger, string path, int fileCount);
}
