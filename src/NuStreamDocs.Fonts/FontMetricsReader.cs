...
public static class FontMetricsReader
{
    ...
    public static FontMetrics? Read(ReadOnlySpan<byte> fontBytes) =>
        fontBytes.Length >= sizeof(uint) && BinaryPrimitives.ReadUInt32BigEndian(fontBytes) == Woff2Signature
            ? Woff2Reader.TryRead(fontBytes)
            : SfntTableReader.TryRead(fontBytes);
}
