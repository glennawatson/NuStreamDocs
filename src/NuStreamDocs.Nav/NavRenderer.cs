...
internal static class NavRenderer
{
    ...
    /// <summary>Separators that distinguish nested pages from home pages.</summary>
    private static readonly SearchValues<char> PathSeparators = SearchValues.Create("/\\");

    /// <summary>Gets the navigation list open bytes.</summary>
    private static ReadOnlySpan<byte> NavigationListOpen => "<ul class=\"md-nav__list\" data-md-scrollfix>"u8;

    /// <summary>Gets the navigation list close bytes.</summary>
    private static ReadOnlySpan<byte> NavigationListClose => "</ul>"u8;

    /// <summary>Gets the span close bytes.</summary>
    private static ReadOnlySpan<byte> SpanClose => "</span>"u8;
    ...
    [SuppressMessage(
        ...
        Justification = "Mutated through 'ref' chained call sites; analyzer false positive.")]
    internal static void RenderFull(NavTree tree, int activeIndex, IBufferWriter<byte> writer)
    {
        ...
        NavRenderContext ctx = new(tree, chain, false, writer);
        var toggleCounter = 0;
        var root = tree.Nodes[NavTree.RootIndex];
        WriteList(in ctx, root.FirstChildIndex, root.ChildCount, 0, ref toggleCounter);
    }
    ...
    [SuppressMessage(
        ...
        "RCS1118:Mark local variable as const",
        Justification = "Mutated through 'ref' chained call sites; analyzer false positive.")]
    internal static void RenderSidebarFull(NavTree tree, int activeIndex, IBufferWriter<byte> writer)
    {
        Span<int> chainBuffer = stackalloc int[ActiveBranchStackBufferSize];
        var chain = BuildActiveBranchChain(tree, activeIndex, chainBuffer);
        NavRenderContext ctx = new(tree, chain, false, writer);
    ...
    [SuppressMessage(
        ...
        "RCS1118:Mark local variable as const",
        Justification = "Mutated through 'ref' chained call sites; analyzer false positive.")]
    internal static void RenderPruned(NavTree tree, int activeIndex, IBufferWriter<byte> writer)
    {
        Span<int> chainBuffer = stackalloc int[ActiveBranchStackBufferSize];
        var chain = BuildActiveBranchChain(tree, activeIndex, chainBuffer);
        NavRenderContext ctx = new(tree, chain, true, writer);
        var toggleCounter = 0;
        var root = tree.Nodes[NavTree.RootIndex];
        WriteList(in ctx, root.FirstChildIndex, root.ChildCount, 0, ref toggleCounter);
    }
    ...
    [SuppressMessage(
        ...
        "RCS1118:Mark local variable as const",
        Justification = "Mutated through 'ref' chained call sites; analyzer false positive.")]
    internal static void RenderSidebarPruned(NavTree tree, int activeIndex, IBufferWriter<byte> writer)
    {
        Span<int> chainBuffer = stackalloc int[ActiveBranchStackBufferSize];
        var chain = BuildActiveBranchChain(tree, activeIndex, chainBuffer);
        NavRenderContext ctx = new(tree, chain, true, writer);
    ...
    internal static void RenderTabs(NavTree tree, int activeIndex, IBufferWriter<byte> writer) =>
        RenderTabs(tree, activeIndex, writer, false, default);
    ...
    internal static void RenderTabs(
        NavTree tree,
        int activeIndex,
        IBufferWriter<byte> writer,
        bool includeHomeTab,
        ReadOnlySpan<byte> homeTabLabel)
    {
        Span<int> chainBuffer = stackalloc int[ActiveBranchStackBufferSize];
        var chain = BuildActiveBranchChain(tree, activeIndex, chainBuffer);
        ...
        WriteUtf8(
            writer,
            "<nav class=\"md-tabs\" aria-label=\"Tabs\" data-md-component=\"tabs\"><div class=\"md-tabs__inner md-grid\"><ul class=\"md-tabs__list\">"u8);
        var root = tree.Nodes[NavTree.RootIndex];
    ...
    internal static Dictionary<byte[], int> BuildUrlIndex(NavTree tree)
    {
        Dictionary<byte[], int> index = [with(tree.Nodes.Length, ByteArrayComparer.Instance)];
    ...
    private static bool IsTopLevelHomePage(in NavTreeNode node)
    {
        ...
        var relative = node.RelativePath;
        if (relative.IsEmpty)
        {
            return false;
        }

        return relative.Value.AsSpan().IndexOfAny(PathSeparators) < 0
               && (string.Equals(relative.Value, "index.md", StringComparison.OrdinalIgnoreCase)
                   || string.Equals(relative.Value, "README.md", StringComparison.OrdinalIgnoreCase));
    }
    ...
    /// <param name="toggleCounter">Per-render section-toggle counter.</param>
    private static void WriteScopedSidebar(
        in NavRenderContext ctx,
        int homeIdx,
        int activeSectionIdx,
        ref int toggleCounter)
    {
        ...
        {
            WriteUtf8(ctx.Writer, NavigationListOpen);
            if (homeIdx >= 0)
            {
                WriteItem(in ctx, homeIdx, 0, ref toggleCounter);
            }

            for (var i = 0; i < activeSection.ChildCount; i++)
            {
                WriteItem(in ctx, activeSection.FirstChildIndex + i, 0, ref toggleCounter);
            }

            WriteUtf8(ctx.Writer, NavigationListClose);
            return;
        }
        ...
        WriteItem(in ctx, activeSectionIdx, 0, ref toggleCounter);
        WriteUtf8(ctx.Writer, NavigationListClose);
    }
    ...
    private static ReadOnlySpan<byte> ResolveItemOpenTag(in NavTreeNode node, bool active, bool prune)
    {
        if (node.IsSection)
        {
            ...
            return prune ? "<li class=\"md-nav__item md-nav__item--pruned md-nav__item--nested\">"u8 : "<li class=\"md-nav__item md-nav__item--nested\">"u8;
        }
        ...
        return prune ? "<li class=\"md-nav__item md-nav__item--pruned\">"u8 : "<li class=\"md-nav__item\">"u8;
    }
        ...
        ref int toggleCounter)
    {
        ...
        WriteSectionLink(ctx.Tree, nodeIndex, active, ctx.Prune && !active && node.ChildCount > 0, ctx.Writer);
    }
    ...
    private static void WriteSectionWithToggle(
        in NavRenderContext ctx,
        int nodeIndex,
        bool active,
        int level,
        ref int toggleCounter)
    {
        ...
        WriteSectionLink(ctx.Tree, nodeIndex, active, false, writer);
        var linkDetails = active
            ? "<label class=\"md-nav__link md-nav__link--active\" for=\"__nav_"u8
            : "<label class=\"md-nav__link\" for=\"__nav_"u8;
        WriteUtf8(writer, linkDetails);
        WriteLevel(writer, toggleId);
    ...
    /// <param name="writer">UTF-8 sink.</param>
    private static void WriteSectionLink(
        NavTree tree,
        int nodeIndex,
        bool active,
        bool includeChevron,
        IBufferWriter<byte> writer)
    {
        ...
        {
            var linkDetails = active
                ? "<a class=\"md-nav__link md-nav__link--active\" href=\""u8
                : "<a class=\"md-nav__link\" href=\""u8;
            WriteUtf8(writer, linkDetails);
        ...
        else
        {
            WriteUtf8(
                writer,
                active ? "<span class=\"md-nav__link md-nav__link--active\">"u8 : "<span class=\"md-nav__link\">"u8);
        }
        ...
        WriteUtf8(writer, hasHref ? "</a>"u8 : SpanClose);
    }
    ...
    {
        var linkDetails = active
            ? "<a class=\"md-nav__link md-nav__link--active\" aria-current=\"page\" href=\""u8
            : "<a class=\"md-nav__link\" href=\""u8;
        WriteUtf8(writer, linkDetails);
        WriteRootRelativeHref(writer, node.RelativeUrlBytes, false);
        WriteUtf8(writer, "\">"u8);
    ...
    private static bool TryGetSidebarHref(
        NavTree tree,
        int nodeIndex,
        out ReadOnlySpan<byte> href,
    ...
    /// <param name="writer">UTF-8 sink.</param>
    private static void WriteHomeTab(
        NavTree tree,
        int activeIndex,
        ...
        IBufferWriter<byte> writer)
    {
        ...
        WriteUtf8(
            writer,
            active ? "<li class=\"md-tabs__item md-tabs__item--active\">"u8 : "<li class=\"md-tabs__item\">"u8);
    ...
    /// <param name="writer">UTF-8 sink.</param>
    private static void WriteTabItem(
        NavTree tree,
        int nodeIndex,
        in ReadOnlySpan<int> chain,
        IBufferWriter<byte> writer)
    {
        var node = tree.Nodes[nodeIndex];
    ...
    private static void WriteRootRelativeHref(
        IBufferWriter<byte> writer,
