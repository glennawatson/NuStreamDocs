...
public static class RedirectsOptionsExtensions
{
    ...
    public static RedirectsOptions Add(this in RedirectsOptions options, ReadOnlySpan<byte> from, ReadOnlySpan<byte> to) =>
        Add(options, from, to, permanent: true);
    ...
    /// <returns>The updated options.</returns>
    public static RedirectsOptions Add(this in RedirectsOptions options, ReadOnlySpan<byte> from, ReadOnlySpan<byte> to, bool permanent) =>
        options with { Redirects = ArrayJoiner.Concat(options.Redirects, [new(from.ToArray(), to.ToArray(), permanent)]) };
    ...
    /// <returns>The updated options.</returns>
    public static RedirectsOptions AddHeaders(this in RedirectsOptions options, ReadOnlySpan<byte> pathPattern, params byte[][] headerLines) =>
    ...
    public static RedirectsOptions WithSecurityHeaders(this in RedirectsOptions options) =>
        AddHeaders(options, "/*"u8, [.. "X-Content-Type-Options: nosniff"u8], [.. "Referrer-Policy: strict-origin-when-cross-origin"u8]);
