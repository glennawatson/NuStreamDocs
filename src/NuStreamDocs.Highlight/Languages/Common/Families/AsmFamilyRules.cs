...
internal static class AsmFamilyRules
{
    ...
    internal static Lexer CreateLexer(in AsmFamilyConfig config) =>
    ...
    internal static LexerRule[] Build(in AsmFamilyConfig config)
    {
        ...
        [
            ...
            new(
                static slice => TokenMatchers.MatchIdentifier(
                    slice,
    ...
    private static int MatchLineComment(ReadOnlySpan<byte> slice, SearchValues<byte> prefixSet) => slice is [] || !prefixSet.Contains(slice[0]) ? 0 : TokenMatchers.LineLength(slice);
    ...
    /// <returns>Length matched, or zero.</returns>
    private static int MatchHexLiteral(ReadOnlySpan<byte> slice, bool hexPrefix) => !hexPrefix ? 0 : TokenMatchers.MatchAsciiHexLiteral(slice, HexBody, CFamilyRules.NoSuffix);
