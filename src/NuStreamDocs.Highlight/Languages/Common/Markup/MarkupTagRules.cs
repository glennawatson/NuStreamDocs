...
namespace NuStreamDocs.Highlight.Languages.Common.Markup;
...
internal static class MarkupTagRules
{
    ...
    [
        ...
        new(LanguageCommon.AttributeName, TokenClass.NameAttribute, LexerRule.NoStateChange)
        {
            FirstBytes = LanguageCommon.AttributeNameFirst
        },

        // Tag name (XML name grammar).
        new(LanguageCommon.TagName, TokenClass.NameClass, LexerRule.NoStateChange)
        {
            FirstBytes = LanguageCommon.TagNameFirst
        },

        // '=' attribute separator.
        new(
            static slice =>
                TokenMatchers.MatchSingleByteOf(
                    slice,
                    LanguageCommon.EqualsFirst),
            TokenClass.Operator,
            LexerRule.NoStateChange) { FirstBytes = LanguageCommon.EqualsFirst },

        // "..." double-quoted attribute value (no escapes).
        new(LanguageCommon.DoubleQuotedStringNoEscape, TokenClass.StringDouble, LexerRule.NoStateChange)
        {
        ...
        new(LanguageCommon.SelfClose, TokenClass.Punctuation, LexerRule.PopState)
        {
        ...
        new(
            static slice => TokenMatchers.MatchSingleByteOf(
                slice,
                LanguageCommon.AngleCloseFirst),
            TokenClass.Punctuation,
            LexerRule.PopState) { FirstBytes = LanguageCommon.AngleCloseFirst }
    ];
}
