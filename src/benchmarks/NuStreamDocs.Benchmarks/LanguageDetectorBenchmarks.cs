...
/// <summary>Throughput + allocation benchmarks for the auto-detect feature in <see cref="HighlightPlugin"/>.</summary>
/// <remarks>
...
/// </list>
/// </remarks>
[DebuggerDisplay("LanguageDetectorBenchmarks: pluginAutoDetectOff={_pluginAutoDetectOff}, pluginAutoDetectOn={_pluginAutoDetectOn}")]
[ShortRunJob]
[MemoryDiagnoser]
public class LanguageDetectorBenchmarks
{
    ...
    /// <summary>Gets the C# language alias shared by the detection fixtures.</summary>
    private static ReadOnlySpan<byte> CSharpLanguage => "csharp"u8;
    ...
    public void Setup()
    {
        ...
        _pageLabeledOnly = WrapInPreCode(_bodyCSharp, CSharpLanguage);
        _pageUnlabeledCSharp = WrapInPreCode(_bodyCSharp, []);
        _pageUnlabeledAmbiguous = WrapInPreCode(_bodyAmbiguous, []);
        ...
        _pluginAutoDetectScoped = new(HighlightOptions.Default with
        {
            ...
            DetectionLanguages = [[.. CSharpLanguage], [.. "powershell"u8]]
        });

        _registry = LexerRegistry.Build();
        _allowAll = [];
        _allowScoped = [[.. CSharpLanguage], [.. "powershell"u8]];
    }

    /// <summary>NeedsRewrite on a page with no code blocks — auto-detect off (baseline single SIMD scan).</summary>
    ...
    /// <summary>NeedsRewrite on a page with no code blocks — auto-detect on (extra union-scan for unlabeled openers).</summary>
    /// <returns>Always false.</returns>
    [MethodImpl(MethodImplOptions.NoInlining)]
    [Benchmark]
    ...
    /// <param name="language">Language alias for the <c>class="language-X"</c> attribute, or empty for an unlabeled block.</param>
    /// <returns>Page-shaped HTML bytes.</returns>
    private static byte[] WrapInPreCode(ReadOnlySpan<byte> body, ReadOnlySpan<byte> language)
    {
        byte[] prefix = language.IsEmpty
            ? [.. "<p>intro</p>\n<pre><code>"u8]
            : [.. "<p>intro</p>\n<pre><code class=\"language-"u8, .. language, .. "\">"u8];
        return [.. prefix, .. body, .. "</code></pre>\n<p>outro</p>"u8];
    }
