...
AMD Ryzen 7 5800X, Linux, .NET 10.0.7, BenchmarkDotNet 0.15.8, Release mode,
three warmups and three measurements. Allocations are total managed bytes per
operation. CPU and allocation profiles use separate EventPipe runs.
...
| **FullStack** (combined plugins) | **87.86 ms** | 13.21 MB | 417 µs |
| EverythingStack (kitchen sink) | 65.58 ms | 11.70 MB | 311 µs |

**Throughput**: ~19,000 pages/sec for the parse → render → emit core. ~2,400 pages/sec with the combined plugin configuration.

---

## Synthetic build pipeline

Builds over synthetic Markdown files, including disk reads and output writes. The two page counts exercise different scheduling and per-page overheads.
...
Pure parse + emit hot path with no plugins. The renderer streams to a pooled `IBufferWriter<byte>`; these cases recorded no managed allocation.
