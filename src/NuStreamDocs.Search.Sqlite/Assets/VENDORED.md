...
- Files:
  ...
  - `sql-wasm.wasm` — the sql.js (Emscripten-compiled SQLite, FTS5 enabled) WebAssembly binary, copied verbatim from `dist/sql-wasm.wasm`.
- Refresh: re-run `npm pack sql.js-httpvfs`, copy `dist/index.js` → `sql.js-httpvfs.js`, `dist/sqlite.worker.js` and `dist/sql-wasm.wasm` verbatim, then bump `SqliteAssets.PinnedRuntimeVersion`.
