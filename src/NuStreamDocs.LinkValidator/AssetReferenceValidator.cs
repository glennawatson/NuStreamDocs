...
public static class AssetReferenceValidator
{
    ...
    /// <summary>Per-page validation state. Bundles the five values the resolve / report path
    ...
    /// <param name="Scratch">Caller-owned (pooled) scratch buffer for resolved bytes.</param>
    /// <param name="Sink">Diagnostic accumulator.</param>
    /// <param name="Reported">Per-page set of already-reported resolved targets.</param>
        ...
        HashSet<byte[]> Reported)
    {
        ...
        public void ResolveAndReport(byte[] rawAsset)
        {
        ...
        private void ResolveOverflow(byte[] rawAsset, ReadOnlySpan<byte> pathSpan)
        {
            var sourceDir = LastSlashIndex(Source.PageUrl);
        ...
        /// <summary>Adds a diagnostic only when <paramref name="resolved"/> hasn't already been reported on this page.</summary>
        /// <param name="rawAsset">Original raw asset bytes.</param>
        /// <param name="resolved">Resolved asset path bytes.</param>
        private void ReportOnce(byte[] rawAsset, ReadOnlySpan<byte> resolved)
        {
            if (!Reported.Add([.. resolved]))
            {
                return;
            }
