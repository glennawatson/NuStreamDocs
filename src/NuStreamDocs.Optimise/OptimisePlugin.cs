...
using NuStreamDocs.Optimise.Logging;
...
/// </summary>
/// <remarks>
/// Runs once at <see cref="OnFinaliseAsync"/> after every page is written.
/// Files smaller than <see cref="OptimiseOptions.MinimumBytes"/> are
...
/// </remarks>
public sealed class OptimisePlugin(OptimiseOptions options, ILogger logger) : IDocPlugin
{
    /// <summary>Configured options.</summary>
    private readonly OptimiseOptions _options = ValidateOptions(options);
    ...
    /// <summary>Initializes a new instance of the <see cref="OptimisePlugin"/> class with default options.</summary>
    public OptimisePlugin()
        : this(OptimiseOptions.Default, NullLogger.Instance)
    {
    }

    /// <summary>Initializes a new instance of the <see cref="OptimisePlugin"/> class.</summary>
    /// <param name="options">Plugin options.</param>
    public OptimisePlugin(OptimiseOptions options)
        : this(options, NullLogger.Instance)
    {
    }

    /// <inheritdoc/>
    public string Name => "optimise";
    ...
    {
        if (_options.Formats == OptimiseFormats.None || !Directory.Exists(context.OutputRoot))
        {
            return;
        }
    ...
    internal async Task CompressTreeAsync(string outputRoot, CancellationToken cancellationToken)
    {
        ...
        OptimiseLoggingHelper.LogOptimiseStart(_logger, eligible.Length, outputRoot);
        ...
        OptimiseLoggingHelper.LogOptimiseComplete(_logger, processed, bytesSaved);
    }
    ...
    private static OptimiseOptions ValidateOptions(OptimiseOptions opts)
    {
    ...
    private string[] EnumerateEligible(string root)
    {
        ...
        foreach (var path in Directory.EnumerateFiles(root, "*", SearchOption.AllDirectories))
        {
            ...
            {
                OptimiseLoggingHelper.LogFileSkipped(_logger, path, "extension not in compressible set");
                continue;
            }
            ...
            {
                OptimiseLoggingHelper.LogFileSkipped(_logger, path, "below minimum size");
                continue;
            }
    ...
    private async Task<long> CompressOneAsync(string path, CancellationToken cancellationToken)
    {
        ...
        long saved = 0;
        if ((_options.Formats & OptimiseFormats.Gzip) == OptimiseFormats.Gzip)
        {
            ...
            OptimiseLoggingHelper.LogFileProcessed(_logger, path, originalBytes, gzBytes);
        }

        if ((_options.Formats & OptimiseFormats.Brotli) != OptimiseFormats.Brotli)
        {
            return saved;
        }
        ...
        OptimiseLoggingHelper.LogFileProcessed(_logger, path, originalBytes, brBytes);
        return saved;
    }
}
