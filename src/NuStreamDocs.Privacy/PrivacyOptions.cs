...
/// <remarks>Mirrors the most-used knobs of mkdocs-material's privacy plugin, scoped to asset externalisation.</remarks>
/// <param name="Enabled">Master switch. When false, the plugin is a no-op.</param>
/// <param name="AssetDirectory">Forward-slash relative directory the externalised assets are written to (default <c>assets/external</c>).</param>
...
/// <param name="HostsAllowed">When non-empty, only URLs whose host matches this list are localised. Empty
/// (the default) means "localise everything not on <see cref="HostsToSkip"/>".</param>
...
/// response) raises an exception during finalise; false (the default) swallows individual
...
/// paths on a host whose other paths should remain external (e.g. localise
