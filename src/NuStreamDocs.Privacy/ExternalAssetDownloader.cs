...
internal static class ExternalAssetDownloader
{
    ...
    /// <summary>Client used to retrieve external assets across download batches.</summary>
    private static readonly HttpClient SharedHttpClient = new(new SocketsHttpHandler
    {
        PooledConnectionLifetime = PooledConnectionLifetime,
        PooledConnectionIdleTimeout = PooledConnectionIdleTimeout,
        EnableMultipleHttp2Connections = true,
    }) { Timeout = Timeout.InfiniteTimeSpan, DefaultRequestVersion = HttpVersion.Version20, DefaultVersionPolicy = HttpVersionPolicy.RequestVersionOrLower, };
    ...
    internal static async Task<string[]> DownloadAllAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (settings.Timeout != Timeout.InfiniteTimeSpan)
        {
            ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(settings.Timeout, TimeSpan.Zero);
            ArgumentOutOfRangeException.ThrowIfGreaterThan(settings.Timeout, TimeSpan.FromMilliseconds(int.MaxValue));
        }

        await using var perHostLimiter = BuildPerHostLimiter();
        DownloadEnvironment environment =
            new(SharedHttpClient, BuildPipeline(settings.MaxRetries, perHostLimiter), registry, filter, settings.Timeout);

        ConcurrentBag<string> failures = [];
        HashSet<byte[]> processed = [with(ByteArrayComparer.Instance)];
        ...
        HashSet<byte[]> processed)
    {
        ...
        List<(UrlPath Url, FilePath LocalPath)> pendingBuffer = [with(snapshot.Length)];
        ...
        CancellationToken cancellationToken)
    {
        ...
        await Parallel.ForEachAsync(
                pending,
                new ParallelOptions { MaxDegreeOfParallelism = ctx.Settings.Parallelism, CancellationToken = cancellationToken, },
                async (entry, ct) =>
                {
        ...
        LogInvokerHelper.Invoke(
            ...
            (Iteration: iterationNumber, Processed: pending.Length, Failed: ctx.Failures.Count - iterationFailureBaseline, Timestamp: iterationTimestamp),
            static (logger, state) =>
            {
                var elapsedSeconds = Stopwatch.GetElapsedTime(state.Timestamp).TotalSeconds;
                PrivacyLoggingHelper.LogIterationComplete(logger, state.Iteration, state.Processed, state.Failed, elapsedSeconds);
            });
    }
    ...
    private static ResiliencePipeline<HttpResponseMessage> BuildPipeline(
        ...
        new ResiliencePipelineBuilder<HttpResponseMessage>()
            .AddRateLimiter(new RateLimiterStrategyOptions { RateLimiter = args => perHostLimiter.AcquireAsync(args.Context, 1, args.Context.CancellationToken), })
            .AddRetry(new()
            {
            ...
            using var response = await environment.Pipeline.ExecuteAsync(
                    static async (ctx, state) =>
                    {
                        ...
                        return await state.Client
                            .GetAsync(state.Uri, HttpCompletionOption.ResponseHeadersRead, timeoutCts.Token)
                            .ConfigureAwait(false);
                    },
                    context,
                    new GetCallState(environment.Client, uri, environment.Timeout))
                .ConfigureAwait(false);
    ...
    internal readonly record struct DownloadSettings(int Parallelism, TimeSpan Timeout, int MaxRetries);
    ...
    internal readonly record struct DownloadTarget(UrlPath Url, FilePath OutputPath, FilePath CachePath);
    ...
    /// <param name="Timeout">Timeout for each HTTP request attempt.</param>
    internal readonly record struct DownloadEnvironment(
        HttpClient Client,
    ...
    /// <param name="Timeout">Timeout for the HTTP request attempt.</param>
    private readonly record struct GetCallState(HttpClient Client, Uri Uri, TimeSpan Timeout);
