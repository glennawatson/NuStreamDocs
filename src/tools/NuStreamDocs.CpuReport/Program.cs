...
namespace NuStreamDocs.CpuReport;
...
/// </summary>
/// <remarks>
/// Sister tool to <c>smkd-allocreport</c> (which consumes <c>GcVerbose</c> traces). CPU
/// sampling fires roughly once per millisecond per running managed thread, so absolute counts
/// are noisy on a single trace; the *relative* ranking between methods is what carries signal.
/// </remarks>
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        if (args.Length is 0)
        {
            Console.Error.WriteLine("usage: smkd-cpureport <path-to-trace.nettrace> [topN]");
    ...
    private static AggregatedStats AggregateSamples(string tracePath)
    {
        ...
        try
        {
            ...
            // EventPipe's CpuSampling profile surfaces as
            // "Microsoft-DotNETRuntime-SampleProfiler/Thread/Sample" events. Each sample is
            // one tick of the sampling clock; the bottom of its call stack is the currently-
            // executing method. We listen via AllEvents + provider filter to avoid pulling
            // in the (private-API) typed parser.
