...
namespace NuStreamDocs.AllocReport;
...
internal static class Program
{
    ...
    internal static int Main(string[] args)
    {
        if (args.Length == 0)
        {
            Console.Error.WriteLine("usage: smkd-allocreport <path-to-trace.nettrace> [topN]");
        ...
        WriteTypeTable(stats, topN, Console.Out);
    ...
    {
        Dictionary<string, AllocStat> byType = [with(StringComparer.Ordinal)];
    ...
    {
        writer.WriteLine($"# Allocation Report -- {Path.GetFileName(tracePath)}");
        writer.WriteLine();
        writer.WriteLine($"- Total sampled allocation bytes: **{FormatBytes(stats.TotalBytes)}**");
        writer.WriteLine($"- Total sample events: **{stats.TotalSamples:N0}**");
        writer.WriteLine(
    ...
    /// <param name="writer">Report output destination.</param>
    private static void WriteTypeTable(AggregatedStats stats, int topN, TextWriter writer)
    {
        writer.WriteLine($"## Top {topN} types by sampled bytes");
        writer.WriteLine();
        writer.WriteLine("| Type | Sampled bytes | % | Samples |");
        writer.WriteLine("|---|---:|---:|---:|");
        KeyValuePair<string, AllocStat>[] types = [.. stats.ByType];
        Array.Sort(types, static (left, right) => right.Value.Bytes.CompareTo(left.Value.Bytes));
        for (var i = 0; i < Math.Min(topN, types.Length); i++)
        {
            var (type, stat) = types[i];
            var pct = stats.TotalBytes == 0 ? 0D : stat.Bytes * PercentMultiplier / stats.TotalBytes;
            writer.WriteLine($"| `{Escape(type)}` | {FormatBytes(stat.Bytes)} | {pct:F1}% | {stat.Samples:N0} |");
        }
    ...
    {
        writer.WriteLine($"## Top {topN} call stacks by sampled bytes (depth {StackDepth})");
        writer.WriteLine();
        writer.WriteLine("| Top type | Sampled bytes | % | Samples | Stack |");
        writer.WriteLine("|---|---:|---:|---:|---|");
        KeyValuePair<string, StackStat>[] stacks = [.. stats.ByStack];
        Array.Sort(stacks, static (left, right) => right.Value.Bytes.CompareTo(left.Value.Bytes));
    ...
    private record struct AllocStat
    {
        ...
        /// <summary>Gets or sets number of GCAllocationTick samples that named this type.</summary>
