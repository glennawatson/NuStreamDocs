...
{
    /// <summary>Writes the locator per AGLC4 conventions directly to <paramref name="writer"/>.</summary>
    /// <param name="locator">Source locator.</param>
    /// <param name="writer">UTF-8 sink.</param>
    public static void Write(CitationLocator locator, IBufferWriter<byte> writer)
    {
        if (locator.Kind is LocatorKind.Paragraph)
        {
            Aglc4Writer.WriteBytes("["u8, writer);
            Aglc4Writer.WriteString(locator.Value, writer);
            Aglc4Writer.WriteBytes("]"u8, writer);
            return;
        }
