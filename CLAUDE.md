...
- **Path-typed public APIs use `NuStreamDocs.Common.DirectoryPath` / `FilePath`, never `string`, `DirectoryInfo`/`FileInfo`, or `Nuke.Common.IO.AbsolutePath`.** Both wrappers are `readonly record struct`s holding a single `string` field — same memory cost as a string, free `Equals`/`GetHashCode`, never touch the filesystem. Implicit conversion to `string` keeps BCL interop trivial (`Directory.Exists(path)` works). Use `/` to join: `inputRoot / "guide" / "intro.md"`. `DirectoryInfo`/`FileInfo` allocate heavier state and silently cache filesystem metadata — avoid them. Validate with `ArgumentException.ThrowIfNullOrEmpty(path.Value)` at API boundaries that don't accept `default`. Internal-only paths inside core may stay `string` until they cross an assembly boundary.
