...
- **XML `<summary>` describes what the type / method / property does, not how it's implemented.** Summaries on every public *and* internal type, method, and property answer "what is this for?" — implementation detail (which collection it wraps, which algorithm it uses, table sizes, GC behaviour, hashing strategy, whether something is `Frozen*` / pooled / cached, the data-structure layout) belongs in `<remarks>`, not in the summary. Wrong: `Byte-keyed icon lookup backed by a Dictionary<byte[], (int, int)> over a concatenated SVG blob with an IAlternateEqualityComparer for span lookups…`. Right: `Resolves a Material Design icon name to its SVG path-data bytes.` Same rule for methods (`Returns the local path for the URL.` not `xxHash3-keyed concurrent dictionary lookup, falling back to BuildLocalPath when the key is absent.`) and properties (`Gets the page URL.` not `Returns the cached UTF-8 bytes lazily computed from _path.`). When an internal method genuinely needs an implementation explanation (a non-obvious algorithm, a workaround, a perf-driven choice), put that explanation in a `<remarks>` block — never in the `<summary>`.
